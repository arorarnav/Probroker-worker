...
def build_report(rows: list[dict], matches: list[dict], out_path: str,
    ...
    ws = wb.active
    ws.title = "Listings"
    headers = ["Date Posted", "Times Posted", "Posted By", "Type", "Category", "Location", "Notes",
    ...
    for i, r in enumerate(rows, start=2):
        ...
        ws.cell(row=i, column=5, value=r.get("category", "sale")).font = FONT_NORMAL
        ws.cell(row=i, column=6, value=r.get("location")).font = FONT_NORMAL
        ws.cell(row=i, column=7, value=r.get("notes")).font = FONT_NORMAL
        ws.cell(row=i, column=8, value=r.get("size")).font = FONT_NORMAL
        ws.cell(row=i, column=9, value=r.get("price")).font = FONT_NORMAL
        ws.cell(row=i, column=10, value=r.get("contact")).font = FONT_NORMAL
        ws.cell(row=i, column=11, value=f"=TODAY()-DATEVALUE(A{i})").font = FONT_NORMAL
        ws.cell(row=i, column=12, value=(
            f'=IF(K{i}<=$O$1,"Active",IF(K{i}<=$O$2,"Aging","Stale"))'
        )).font = FONT_NORMAL
        for col in range(1, 13):
            ws.cell(row=i, column=col).border = BORDER

    last_row = len(rows) + 1
    widths = {1: 12, 2: 12, 3: 22, 4: 10, 5: 10, 6: 34, 7: 34, 8: 20, 9: 20, 10: 18, 11: 10, 12: 10}
    for col, w in widths.items():
        ws.column_dimensions[get_column_letter(col)].width = w
    ws.freeze_panes = "A2"
    ws.add_table(Table(displayName="Listings", ref=f"A1:L{last_row}",
